...
PSOProperty OutlineBlue::CreatePipelineStateObject()
{
	...
	property.pixelShaderBlob = CompileShader(L"Resources/shader/OutlineBlue.PS.hlsl",
...
void OutlineBlue::CommandRootParameter(PostProcess* postProcess)
{
	DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();
...
std::vector<D3D12_DESCRIPTOR_RANGE> OutlineBlue::CreateDescriptorRange()
{
...
std::vector<D3D12_ROOT_PARAMETER> OutlineBlue::CreateRootParamerter(std::vector<D3D12_DESCRIPTOR_RANGE>& descriptorRange)
{
...
std::vector<D3D12_STATIC_SAMPLER_DESC> OutlineBlue::CreateSampler()
{
...
D3D12_ROOT_SIGNATURE_DESC OutlineBlue::CreateRootSignature(PSOProperty& pso, std::vector<D3D12_ROOT_PARAMETER>& rootParameters, std::vector<D3D12_STATIC_SAMPLER_DESC>& samplers)
{
	HRESULT hr;
	// RootSignature作成
...
D3D12_INPUT_LAYOUT_DESC OutlineBlue::SetInputLayout()
{
...
D3D12_BLEND_DESC OutlineBlue::SetBlendState()
{
	// blendStateの設定
	//すべての色要素を書き込む
	// blendStateの設定
...
D3D12_RASTERIZER_DESC OutlineBlue::SetRasterizerState()
{
	// RasiterzerStateの設定
...
D3D12_DEPTH_STENCIL_DESC OutlineBlue::CreateDepth()
{
