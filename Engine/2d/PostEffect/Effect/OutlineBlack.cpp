...
PSOProperty OutlineBlack::CreatePipelineStateObject()
{
	...
	property.pixelShaderBlob = CompileShader(L"Resources/shader/OutlineBlack.PS.hlsl",
...
void OutlineBlack::CommandRootParameter(PostProcess* postProcess)
{
	DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();
...
std::vector<D3D12_DESCRIPTOR_RANGE> OutlineBlack::CreateDescriptorRange()
{
...
std::vector<D3D12_ROOT_PARAMETER> OutlineBlack::CreateRootParamerter(std::vector<D3D12_DESCRIPTOR_RANGE>& descriptorRange)
{
...
std::vector<D3D12_STATIC_SAMPLER_DESC> OutlineBlack::CreateSampler()
{
...
D3D12_ROOT_SIGNATURE_DESC OutlineBlack::CreateRootSignature(PSOProperty& pso, std::vector<D3D12_ROOT_PARAMETER>& rootParameters, std::vector<D3D12_STATIC_SAMPLER_DESC>& samplers)
{
	HRESULT hr;
	// RootSignature作成
...
D3D12_INPUT_LAYOUT_DESC OutlineBlack::SetInputLayout()
{
...
D3D12_BLEND_DESC OutlineBlack::SetBlendState()
{
	// blendStateの設定
	//すべての色要素を書き込む
	// blendStateの設定
...
D3D12_RASTERIZER_DESC OutlineBlack::SetRasterizerState()
{
	// RasiterzerStateの設定
...
D3D12_DEPTH_STENCIL_DESC OutlineBlack::CreateDepth()
{
