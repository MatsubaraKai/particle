...
PSOProperty OutlinePurple::CreatePipelineStateObject()
{
	...
	property.pixelShaderBlob = CompileShader(L"Resources/shader/OutlinePurple.PS.hlsl",
...
void OutlinePurple::CommandRootParameter(PostProcess* postProcess)
{
	DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();
...
std::vector<D3D12_DESCRIPTOR_RANGE> OutlinePurple::CreateDescriptorRange()
{
...
std::vector<D3D12_ROOT_PARAMETER> OutlinePurple::CreateRootParamerter(std::vector<D3D12_DESCRIPTOR_RANGE>& descriptorRange)
{
...
std::vector<D3D12_STATIC_SAMPLER_DESC> OutlinePurple::CreateSampler()
{
...
D3D12_ROOT_SIGNATURE_DESC OutlinePurple::CreateRootSignature(PSOProperty& pso, std::vector<D3D12_ROOT_PARAMETER>& rootParameters, std::vector<D3D12_STATIC_SAMPLER_DESC>& samplers)
{
	HRESULT hr;
	// RootSignature作成
...
D3D12_INPUT_LAYOUT_DESC OutlinePurple::SetInputLayout()
{
...
D3D12_BLEND_DESC OutlinePurple::SetBlendState()
{
	// blendStateの設定
	//すべての色要素を書き込む
	// blendStateの設定
...
D3D12_RASTERIZER_DESC OutlinePurple::SetRasterizerState()
{
	// RasiterzerStateの設定
...
D3D12_DEPTH_STENCIL_DESC OutlinePurple::CreateDepth()
{
