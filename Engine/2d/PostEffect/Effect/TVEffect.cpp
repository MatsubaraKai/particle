...
PSOProperty TVEffect::CreatePipelineStateObject()
{
	...
	property.pixelShaderBlob = CompileShader(L"Resources/shader/TVEffect.PS.hlsl",
...
void TVEffect::CommandRootParameter(PostProcess* postProcess)
{
	DirectXCommon* sDirectXCommon = DirectXCommon::GetInstance();
...
std::vector<D3D12_DESCRIPTOR_RANGE> TVEffect::CreateDescriptorRange()
{
...
std::vector<D3D12_ROOT_PARAMETER> TVEffect::CreateRootParamerter(std::vector<D3D12_DESCRIPTOR_RANGE>& descriptorRange)
{
...
std::vector<D3D12_STATIC_SAMPLER_DESC> TVEffect::CreateSampler()
{
...
D3D12_ROOT_SIGNATURE_DESC TVEffect::CreateRootSignature(PSOProperty& pso, std::vector<D3D12_ROOT_PARAMETER>& rootParameters, std::vector<D3D12_STATIC_SAMPLER_DESC>& samplers)
{
	HRESULT hr;
	// RootSignature作成
...
D3D12_INPUT_LAYOUT_DESC TVEffect::SetInputLayout()
{
...
D3D12_BLEND_DESC TVEffect::SetBlendState()
{
	// blendStateの設定
	//すべての色要素を書き込む
	// blendStateの設定
...
D3D12_RASTERIZER_DESC TVEffect::SetRasterrizerState()
{
	// RasiterzerStateの設定
...
D3D12_DEPTH_STENCIL_DESC TVEffect::CreateDepth()
{
